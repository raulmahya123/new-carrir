      ...
      <main class="p-4 sm:p-6 space-y-6">
        ...
        <div class="rounded-2xl bg-gradient-to-r from-sky-500 to-emerald-500 p-[1px]">
          <div class="rounded-2xl bg-white/70 backdrop-blur px-5 py-4 flex items-center justify-between">
            <div>
              <h1 class="text-xl sm:text-2xl font-semibold tracking-tight">Tambah Kategori</h1>
              <p class="text-sm text-slate-600 mt-1">Buat kategori pekerjaan baru</p>
            </div>
            <a href="{{ route('admin.categories.index') }}"
          ...
          <div class="rounded-2xl bg-white/80 backdrop-blur p-5 max-w-xl">
            <form method="POST" action="{{ route('admin.categories.store') }}" class="space-y-5">
              ...
              <div>
                ...
                <p class="text-xs text-slate-500 mt-1">Slug akan terisi otomatis dari nama (bisa diedit).</p>
                @error('slug')
                  <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
              </div>
  ...
  <script>
    ...
    function toSlug(v){
      return (v || '')
        ...
        .replace(/^-+|-+$/g,'')                          // trim -
        .substring(0,120);                               // batas aman
    }
    $name?.addEventListener('input', () => {
      // hanya timpa kalau user belum mengubah slug manual
