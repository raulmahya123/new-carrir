      ...
      <main class="p-4 sm:p-6 space-y-6">
        <div class="rounded-2xl bg-gradient-to-r from-sky-500 to-emerald-500 p-[1px]">
          <div class="rounded-2xl bg-white/70 backdrop-blur px-5 py-4">
            <h1 class="text-xl sm:text-2xl font-semibold tracking-tight">Tambah Location</h1>
            <p class="text-sm text-slate-600 mt-1">Masukkan data lokasi baru</p>
          </div>
        </div>
        ...
        <form method="POST" action="{{ route('admin.locations.store') }}"
          ...
          <div>
            ...
            <input type="text" name="region" value="{{ old('region') }}" class="w-full border p-2 rounded">
            @error('region')<div class="text-red-600 text-sm">{{ $message }}</div>@enderror
          </div>

          <div>
            <label class="block mb-1 font-medium">Country</label>
            <input type="text" name="country" value="{{ old('country','ID') }}" class="w-full border p-2 rounded">
          ...
          <div class="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" name="lat" value="{{ old('lat') }}" class="w-full border p-2 rounded" placeholder="-6.2000">
              @error('lat')<div class="text-red-600 text-sm">{{ $message }}</div>@enderror
            </div>
            <div>
              <label class="block mb-1 font-medium">Longitude</label>
              <input type="text" name="lng" value="{{ old('lng') }}" class="w-full border p-2 rounded" placeholder="106.8166">
